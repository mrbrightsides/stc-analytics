import os, io, json, re, hashlib, traceback
...
def read_csv_any(uploaded):
    ...
    try:
        ...
        return pd.read_csv(io.StringIO(data.decode("utf-8", "ignore")), engine="python", on_bad_lines="skip")
...
def render_cost_page():
    ...
    def map_csv_cost(df_raw: pd.DataFrame) -> pd.DataFrame:
        ...
        df["project"] = "STC"
        df["timestamp"] = pd.to_datetime(df.get("timestamp"), errors="coerce").fillna(pd.Timestamp.utcnow())
        ...
        tx = df.get("tx_hash")
        fn = df.get("function_name")
        tx = (tx.astype(str).fillna("") if tx is not None else pd.Series([""]*len(df)))
        fn = (fn.astype(str).fillna("") if fn is not None else pd.Series([""]*len(df)))
        df["id"] = tx + "::" + fn
    ...
    with st.expander("Ingest data (NDJSON/CSV) → DuckDB", expanded=False):
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.download_button("⬇️ Template CSV (Vision)", data=csv_bytes(tpl_cost),
                               file_name="vision_template.csv", mime="text/csv", use_container_width=True)
        with c2:
            sample = [{
                "id":"demo::bookHotel","project":"STC","network":"Sepolia",
                "timestamp":pd.Timestamp.utcnow().isoformat(),"tx_hash":"0xabc123...",
            ...
            st.download_button("⬇️ Contoh NDJSON (Vision)",
                               ...
                               file_name="vision_sample.ndjson", mime="application/x-ndjson", use_container_width=True)
        ...
        if nd is not None:
            ...
            if rows:
                ...
                if "id" not in d.columns:
                    tx = d.get("tx_hash", "").astype(str).fillna("")
                    fn = d.get("function_name", "").astype(str).fillna("")
                    d["id"] = tx + "::" + fn
    ...
    no_new = (st.session_state.get("nd_cost") is None and st.session_state.get("csv_cost") is None)
    if no_new and not want_load:
    ...
    st.download_button("⬇️ Download CSV (All)", data=csv_bytes(df), file_name="vision_costs_all.csv", mime="text/csv", use_container_width=True)

    # Filters & charts
    UNPARSED_LABEL = "⚠ Unparsed Function"

    dfb = df.copy()
    dfb["ts"] = pd.to_datetime(dfb["timestamp"], errors="coerce")
    dfb["fn_raw"] = dfb["function_name"]
    dfb["fn"] = dfb["fn_raw"].fillna(UNPARSED_LABEL).replace({"(unknown)": UNPARSED_LABEL})
    dfb["cost_idr_num"] = pd.to_numeric(dfb.get("cost_idr", 0), errors="coerce").fillna(0)
    dfb["gas_used_num"] = pd.to_numeric(dfb.get("gas_used", 0), errors="coerce").fillna(0)
    dfb["gas_price_num"] = pd.to_numeric(dfb.get("gas_price_wei", 0), errors="coerce").fillna(0)
    ...
    with fc1:
        dmin = dfb["ts"].min(); dmax = dfb["ts"].max()
        date_range = st.date_input("Tanggal", value=(None if pd.isna(dmin) else dmin.date(), None if pd.isna(dmax) else dmax.date()))
    with fc2:
        f_net = st.selectbox("Network", ["(All)"] + sorted(dfb["network"].dropna().astype(str).unique().tolist()), index=0)
    with fc3:
        f_fn = st.selectbox("Function", ["(All)"] + sorted(dfb["fn"].dropna().astype(str).unique().tolist()), index=0)
    ...
    if isinstance(date_range, (list, tuple)) and len(date_range) == 2:
        ...
        if start: dfp = dfp[dfp["ts"] >= pd.Timestamp(start)]
        if end:   dfp = dfp[dfp["ts"] < (pd.Timestamp(end) + pd.Timedelta(days=1))]
    if f_net != "(All)":
        dfp = dfp[dfp["network"] == f_net]
    if f_fn != "(All)":
        dfp = dfp[dfp["fn"] == f_fn]
    if hide_unknown or (f_fn != "(All)"):
        dfp = dfp[dfp["fn"] != UNPARSED_LABEL]

    # Stats & downloads
    df_stats = dfb.copy()
    if isinstance(date_range, (list, tuple)) and len(date_range) == 2:
        start, end = date_range
        if start: df_stats = df_stats[df_stats["ts"] >= pd.Timestamp(start)]
        if end:   df_stats = df_stats[df_stats["ts"] < (pd.Timestamp(end) + pd.Timedelta(days=1))]
    if f_net != "(All)":
        df_stats = df_stats[df_stats["network"] == f_net]
    if f_fn != "(All)":
        df_stats = df_stats[df_stats["fn"] == f_fn]

    total_rows_stats = len(df_stats)
    unparsed_count = int((df_stats["fn"] == UNPARSED_LABEL).sum())
    pct_unparsed = (unparsed_count / total_rows_stats * 100.0) if total_rows_stats > 0 else 0.0

    b1, b2, b3 = st.columns([2,1,1])
    with b1:
        st.caption(
            f"Menampilkan **{len(dfp):,}** transaksi"
    ...
    with b2:
        ...
        st.download_button("⬇️ Download CSV (Filtered)", data=csv_bytes(dfp.drop(columns=helper_cols, errors="ignore")),
                           file_name="vision_filtered.csv", mime="text/csv", use_container_width=True)
    with b3:
        df_unparsed = df_stats[df_stats["fn"] == UNPARSED_LABEL]
        st.download_button("⬇️ Unparsed CSV", data=csv_bytes(df_unparsed.drop(columns=helper_cols, errors="ignore")),
                           file_name="vision_unparsed_filtered.csv", mime="text/csv", use_container_width=True,
                           disabled=df_unparsed.empty)

    # Charts
    g1, g2 = st.columns(2)
    with g1:
        ts = dfp.dropna(subset=["ts"]).sort_values("ts")
        if not ts.empty:
            y = "cost_idr_num"
            if do_smooth and len(ts) >= 7:
                ts = ts.assign(cost_smooth=ts.groupby("network")[y].transform(lambda s: s.rolling(7, min_periods=1).mean()))
                y = "cost_smooth"
            fig = px.line(ts, x="ts", y=y, color="network", markers=not do_smooth,
    ...
    with g2:
        by_fn = (dfp.groupby("fn", as_index=False)["cost_idr_num"].sum()
                    .sort_values("cost_idr_num", ascending=False).head(15))
        if not by_fn.empty:
            fig = px.bar(by_fn, x="fn", y="cost_idr_num", color="fn", text_auto=True,
                         title="Total Biaya per Function (Rp) — Top 15",
                         labels={"fn":"Function", "cost_idr_num":"Total Biaya (Rp)"})
            fig.update_xaxes(categoryorder="total descending")
            st.plotly_chart(fig, use_container_width=True)

    sc = dfp[(dfp["gas_used_num"] > 0) & (dfp["gas_price_num"] > 0)].copy()
    if not sc.empty:
        ...
        fig = px.scatter(sc, x="gas_used_num", y="gas_price_num", size="cost_idr_num", color="network",
...
def render_swc_page():
    ...
    with st.expander("Ingest CSV/NDJSON SWC Findings", expanded=False):
        ...
        with col_dl1:
            st.download_button("⬇️ Template CSV (SWC)",
        ...
        with col_dl2:
            sample_rows = [{
                "finding_id":"", "timestamp":pd.Timestamp.utcnow().isoformat(), "network":"Sepolia",
                "contract":"SmartReservation", "file":"contracts/SmartReservation.sol",
                "line_start":98, "line_end":102, "swc_id":"SWC-105", "title":"Potential issue SWC-105",
                "severity":"Low", "confidence":0.82, "status":"Open", "remediation":"Review and document", "commit_hash":"0xa36e...c5b0"
            }]
            st.download_button("⬇️ Contoh NDJSON (SWC)",
    ...
    want_load = st.session_state.get("load_existing", False)
    no_new = (st.session_state.get("swc_csv") is None and st.session_state.get("swc_nd") is None)
    if no_new and not want_load:
    ...
    with b2:
        st.download_button("⬇️ Download CSV (Filtered)",
                           data=csv_bytes(swc_plot.drop(columns=["ts","sev","conf_num"], errors="ignore")),
                           file_name="swc_findings_filtered.csv", mime="text/csv", use_container_width=True)
    ...
    pivot = swc_plot.pivot_table(index="swc_id", columns="sev", values="finding_id", aggfunc="count", fill_value=0)
    if not pivot.empty:
    ...
    st.download_button("⬇️ Download tabel di atas (CSV)", data=csv_bytes(swc_plot[detail_cols]),
                       file_name="swc_table_filtered.csv", mime="text/csv", use_container_width=True)
...
def render_bench_page():
    ...
    with st.expander("Ingest CSV Bench (runs & tx)", expanded=False):
        ...
        with col1:
            ...
            if runs is not None:
                d = read_csv_any(runs)
                cols = ["run_id","timestamp","network","scenario","contract","function_name",
        ...
        with col2:
            ...
            if tx is not None:
                d = read_csv_any(tx)
                cols = ["run_id","tx_hash","submitted_at","mined_at","latency_ms","status",
        ...
        with dcol1:
            st.download_button("⬇️ Template bench_runs.csv",
                               data=csv_bytes(pd.DataFrame(columns=runs_cols).head(0)),
                               file_name="bench_runs_template.csv", mime="text/csv", use_container_width=True)
        with dcol2:
            st.download_button("⬇️ Template bench_tx.csv",
                               data=csv_bytes(pd.DataFrame(columns=tx_cols).head(0)),
                               file_name="bench_tx_template.csv", mime="text/csv", use_container_width=True)

    want_load = st.session_state.get("load_existing", False)
    no_new = ((st.session_state.get("runs_csv") is None) and (st.session_state.get("tx_csv") is None))
    if no_new and not want_load:
    ...
    with fc4:
        f_fn = st.selectbox("Function", ["(All)"] + sorted(base["function_name"].dropna().astype(str).unique().tolist()), index=0)
    ...
    k3.metric("Success Rate", f"{((plot['succ'].mean()*100) if len(plot) else 0.0):.1f}%")

    c1, c2 = st.columns(2)
    with c1:
        fig = px.line(plot.sort_values("concurrency"), x="concurrency", y="tps_avg", color="scenario",
                      markers=True, title="TPS vs Concurrency",
                      labels={"concurrency":"Concurrency","tps_avg":"TPS Avg","scenario":"Scenario"})
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        lat = plot.melt(id_vars=["concurrency","scenario"], value_vars=["p50_ms","p95_ms"],
                        var_name="metric", value_name="latency_ms")
        fig = px.line(lat.sort_values("concurrency"), x="concurrency", y="latency_ms", color="metric",
