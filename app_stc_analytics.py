...
COST_COLS = [
    ...
    "Block","Gas Used","Gas Price (Gwei)","Estimated Fee (ETH)","Estimated Fee (Rp)","Status"
]

def generate_dummy_cost(n=100):
...
def normalize_cost_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reorder (keep only columns that exist)
...
if uploaded_file is not None:
    try:
        ...
        st.success(f"✅ NDJSON dimuat: {len(df_cost)} baris. Contoh timestamp: {df_cost['Timestamp'].head(3).tolist()}")
    except Exception as e:
        st.warning(f"⚠️ Gagal parsing NDJSON (pakai dummy). Error: {e}")
        df_cost = generate_dummy_cost()
else:
    df_cost = generate_dummy_cost()
    st.info("ℹ️ Menampilkan data dummy karena belum ada file yang di-upload.")
