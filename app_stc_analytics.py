...
# --- Konfigurasi kolom yang wajib ada ---
REQUIRED_RUNS = {
    "run_id","timestamp","network","scenario","contract","function_name",
    "concurrency","tx_per_user","tps_avg","tps_peak","p50_ms","p95_ms","success_rate"
}
...
def read_csv_safe(uploaded_file):
    if not uploaded_file:
        return None, "Belum di-upload."
    try:
        df = pd.read_csv(uploaded_file)
        return df, None
    except Exception as e:
        return None, f"Gagal baca CSV: {e}"

def has_required_cols(df, required_cols):
    return df is not None and required_cols.issubset(set(df.columns))
...
# --- Uploaders (punya kamu) ---
left, right = st.columns(2)
with left:
    runs_csv = st.file_uploader("Upload bench_runs.csv", type="csv", key="bench_runs_csv")
with right:
    tx_csv = st.file_uploader("Upload bench_tx.csv", type="csv", key="bench_tx_csv")

# --- Baca file ---
df_runs, err_runs = read_csv_safe(runs_csv)
df_tx, err_tx     = read_csv_safe(tx_csv)

# --- Validasi minimum schema ---
ok_runs = has_required_cols(df_runs, REQUIRED_RUNS)
ok_tx   = has_required_cols(df_tx, REQUIRED_TX)

# --- Validasi JOIN run_id ---
join_ok = False
unknown_run_ids = []
if ok_runs and ok_tx:
    ids_runs = set(df_runs["run_id"].astype(str))
    ids_tx   = set(df_tx["run_id"].astype(str))
    unknown_run_ids = sorted(list(ids_tx - ids_runs))[:5]  # contoh 5 pertama
    join_ok = (len(unknown_run_ids) == 0)

# --- Status chips ---
c1, c2, c3 = st.columns([1,1,1])
with c1: chip("bench_runs.csv", ok_runs)
with c2: chip("bench_tx.csv", ok_tx)
with c3: chip("JOIN run_id", join_ok)

# --- Metrik ringkas ---
m1, m2 = st.columns(2)
with m1:
    st.metric("Rows (runs)", 0 if df_runs is None else len(df_runs))
with m2:
    st.metric("Rows (tx)", 0 if df_tx   is None else len(df_tx))

# --- Pesan panduan / error detail ---
if not runs_csv:
    st.info("🗂️ Unggah **bench_runs.csv**.")
elif err_runs:
    st.error(err_runs)
elif not ok_runs:
    missing = REQUIRED_RUNS - set([] if df_runs is None else df_runs.columns)
    st.error(f"Kolom wajib hilang di **bench_runs.csv**: {sorted(list(missing))}")
...
if not tx_csv:
    ...
    missing = REQUIRED_TX - set([] if df_tx is None else df_tx.columns)
    st.error(f"Kolom wajib hilang di **bench_tx.csv**: {sorted(list(missing))}")

if ok_runs and ok_tx and not join_ok:
    st.warning(
        "Sebagian `run_id` di **bench_tx.csv** tidak ditemukan di **bench_runs.csv**. "
        f"Contoh: {unknown_run_ids} ..."
    )

# --- Gate untuk render chart/analitik di bawah ---
ready = ok_runs and ok_tx and join_ok
if ready:
    st.success("✅ Siap dianalisis. Grafik & metrik diaktifkan di bawah ini.")
    # >>> render chart/analitik kamu di bawah blok ini <<<
else:
    st.info("ℹ️ Unggah **kedua file** dengan struktur kolom yang benar agar analitik aktif.")
